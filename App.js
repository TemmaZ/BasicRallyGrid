Ext.define('CustomApp', {
    ...
    _loadData: function(){
        ...
        for(var index = this.startIndex; index >= this.endIndex; --index){
            var openStart = Ext.create('Rally.data.wsapi.Filter',{
                ...
                operator: '>=',
                value: Ext.Date.format(this.arrayOfReleases[index].data.ReleaseStartDate, 'Y-m-d')
            });
            var openEnd = Ext.create('Rally.data.wsapi.Filter',{
                property: 'CreationDate',
                operator: '<=',
                value: Ext.Date.format(this.arrayOfReleases[index].data.ReleaseDate, 'Y-m-d')
            });
        ...
        console.log("dsdsd", this.arrayOfReleases[0].data);
    ...
    _createChart: function(store){
        ...
        for(i = 0; i < store.getCount(); ++i){
            ...
            }else if(el.Requirement.Release) {
                index = this.releaseById.get(el.Requirement.Release.Name);
            }else {
                var date = el.CreationDate;
                for (var ind = this.startIndex; ind >= this.endIndex; --ind) {
                    var start = this.arrayOfReleases[ind].data.ReleaseStartDate;
                    var end = this.arrayOfReleases[ind].data.ReleaseDate;
